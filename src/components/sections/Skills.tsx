    ...
    <section id="skills" className="py-20 bg-white">
      <div className="container px-4 mx-auto">
        ...
        <div className="mb-16 text-center animate-fade-in">
          <span className="inline-block mb-3 text-sm font-bold tracking-widest text-orange-500 uppercase">
        ...
        <div className="grid max-w-6xl gap-6 mx-auto md:grid-cols-2 lg:grid-cols-4">
          {skillCategories.map((cat, i) => (
            <div
              key={i}
              className={`bg-white rounded-xl p-6 border border-slate-200 hover:border-orange-400 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 animate-fade-in`}
            ...
            >
              ...
              {/* Title */}
              <h3 className="mb-4 text-2xl font-bold text-slate-900">{cat.category}</h3>

              {/* Skills */}
              <ul className="space-y-3">
                {cat.skills.map((skill, j) => (
                  <li key={j} className="flex items-center gap-2 text-slate-700 transition-all hover:translate-x-1 hover:text-orange-500">
